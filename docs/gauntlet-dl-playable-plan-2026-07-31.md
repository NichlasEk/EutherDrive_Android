...
### 2026-07-31: world-global corruption is the next upstream boundary
...
- The f4733 checkpoint's world-global at `0x8016c130` contains overwritten ASCII (`OWTHVOX` and `S_3WAYSHOTVOX`) where the world count, table pointer and selected-world pointer belong. The fallback table at `0x802e1000` is entirely zero. Gameplay state, input, HUD and portal rendering can therefore continue without a consumable world selection.
- A strictly signature-gated late experiment rebuilt 13 entries, loaded the Castle WAD from disk and selected its entry. The resulting 300-frame run was bit-identical (`frameHash=0xb58f5f59`) and still emitted Type 3 only from the UI producer, proving that the world consumer already ran before f4733. The late repair was removed rather than promoted.
- Next: recreate the last pre-`0x400c` checkpoint, watch writes to `0x8016c130..0x8016c14f`, identify the copy/source owner that writes the VOX strings, and prevent or relocate that overlap before the native world selector runs. Then regenerate the gameplay snapshot and require a non-`0x800c4efc` Type 3 producer plus visible arbitrary scene geometry before starting desktop UI snapshot integration.
