...
{
    private const ulong St010ClockHz = 10_000_000;
    ...
    private static readonly bool TraceIo =
        string.Equals(Environment.GetEnvironmentVariable("EUTHERDRIVE_TRACE_ST010_IO"), "1", StringComparison.Ordinal);
    private static readonly bool TraceIoSnes = GetTraceIoSnes();
    private static readonly bool TraceIoDsp = GetTraceIoDsp();
    private static readonly int TraceIoLimit = ParseTraceLimit("EUTHERDRIVE_TRACE_ST010_IO_LIMIT", 2000);
    private int _traceIoCount;

    public Upd77c25(byte[] rom, byte[] sram, bool isPal, bool isSt010 = true)
    {
        (uint[] programRom, ushort[] dataRom) = RomConvert.Convert(rom, isSt010);
        _programRom = programRom;
        _dataRom = dataRom;
        _ram = ConvertSram(sram);
        _registers = new Registers();
        _idling = false;
        _pcMask = 0x3FFF; // 16K opcodes for ST010/ST011 (vs 2K for DSP1)
        _dpMask = 0x07FF; // 2K words RAM for ST010/ST011 (vs 256 words for DSP1)
        _rpMask = 0x07FF; // 2K words data ROM for ST010/ST011 (vs 1K for DSP1)
        ...
        _registers.SwapIoBytes = string.Equals(
            Environment.GetEnvironmentVariable("EUTHERDRIVE_ST010_SWAP_IO_BYTES"),
            "1",
    ...
    public void WriteData(byte value)
    {
        ...
        {
            if (wordComplete)
                Console.WriteLine($"[ST010-IO] SNES write word=0x{word:X4}");
            else
                Console.WriteLine($"[ST010-IO] SNES write byte=0x{value:X2}");
            _traceIoCount++;
        }
        if (!_registers.Status.RequestForMaster)
            _idling = false;
    ...
    public void Tick(ulong masterCyclesElapsed)
    {
        ...
        _masterCyclesProduct += masterCyclesElapsed * St010ClockHz;
    ...
    private void TraceIoRead(byte value)
    {
        ...
        Console.WriteLine($"[ST010-IO] SNES read byte=0x{value:X2} status=0x{status:X2}");
        _traceIoCount++;
    }
    ...
    private void OnUpdWriteData(ushort value)
    {
        if (!TraceIo || !TraceIoDsp || _traceIoCount >= TraceIoLimit)
            return;
        Console.WriteLine($"[ST010-IO] DSP write word=0x{value:X4}");
        _traceIoCount++;
    }
    ...
    {
        string? mode = Environment.GetEnvironmentVariable("EUTHERDRIVE_TRACE_ST010_IO_MODE");
