...
static bool validateCapture(const std::vector<uint32_t>& input,bool allowStatistics=false) {
    ...
    if(draw) {
        ...
        if(m[119]!=0u) throw std::runtime_error("Statistics offset is reserved for native batch assembly");
        if(!m[2] || !m[3] || m[2]>1024 || m[3]>2048 || uint64_t(m[0])+m[2]>1024 || uint64_t(m[1])+m[3]>2048 || (stream?count!=2097152:uint64_t(m[2])*m[3]!=count) || (m[19]!=0xb4779 && !(m[28]==1 && m[19]==0xb4379)) || m[23]!=(stream?1u:0u))
