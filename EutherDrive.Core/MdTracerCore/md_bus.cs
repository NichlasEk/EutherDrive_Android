...
namespace EutherDrive.Core.MdTracerCore
{
    ...
    internal class md_bus
    {
        ...
        private byte ReadOpenBusByte()
        {
            return (byte)((_openBus >> 8) & 0xFF);
        }

        private ushort ReadOpenBusWord()
        {
            return _openBus;
        }
        ...
        {
            _openBus = (ushort)((_openBus & 0x00FF) | (value << 8));
        }
        ...
        private void SetOpenBusFromLong(uint value)
        {
            _openBus = (ushort)((value >> 16) & 0xFFFF);
        }
        ...
        public byte read8(uint in_address)
        {
            ...
            {
                byte val = ReadSramByte(in_address);
                SetOpenBusFromByte(val);
                return val;
            }
            ...
            if (in_address >= 0xA00000 && in_address <= 0xA0FFFF)
            {
                ...
                if (!CanAccessZ80BusRange(in_address, 1))
                {
                    ...
                    byte open = ReadOpenBusByte();
                    SetOpenBusFromByte(open);
                    return open;
                }
            ...
            byte openBus = ReadOpenBusByte();
            SetOpenBusFromByte(openBus);
            return openBus;
        }
        ...
        public ushort read16(uint in_address)
        {
            ...
            {
                ushort val = _sramLock ? (ushort)0x0101 : (ushort)0x0000;
                SetOpenBusFromWord(val);
                return val;
            }
            ...
            if (in_address >= 0xA00000 && in_address <= 0xA0FFFF)
            {
                ...
                SetOpenBusFromWord(mbxMirrorWord);
            ...
            SetOpenBusFromWord(word);
            return word;
        }

            if (string.Equals(Environment.GetEnvironmentVariable("EUTHERDRIVE_TRACE_BUS"), "1", StringComparison.Ordinal))
                Console.WriteLine($"[BUS] read16 open bus addr=0x{in_address:X6}");
            ushort openBus = ReadOpenBusWord();
            SetOpenBusFromWord(openBus);
            return openBus;
        }
        ...
        public uint read32(uint in_address)
        {
            ...
            {
                uint val = _sramLock ? 0x0101_0101u : 0x0000_0000u;
                SetOpenBusFromLong(val);
                return val;
            }
            ...
            uint openBus = ReadOpenBusLong();
            SetOpenBusFromLong(openBus);
            return openBus;
        }
        ...
        {
            // Match MD hardware/jgenesis register lane semantics:
            // - Word writes to A11100/A11200 use bit 8 (MSB lane)
            // - Byte write on even address uses MSB lane
            // - Byte write on odd address uses LSB lane
            // Do not "fallback" to LSB for word writes; that can create spurious
            // BUSREQ/RESET edges when low-byte noise is present.
            if (uds && lds)
        ...
        private void HandleZ80BusReqWrite(uint addr, ushort raw, bool uds, bool lds, uint logValue)
        {
            ...
            {
                // Keep Z80 timebase coherent across BUSREQ edges in all MD paths.
        ...
        private void HandleZ80ResetWrite(uint addr, ushort raw, bool uds, bool lds, uint logValue)
        {
            ...
            if (prev != next && md_main.g_md_z80 != null)
            {
                ...
                else
                {
                    ...
                    {
                        // Keep Z80 aligned before RESET-release start sequence.
                        md_main.SyncZ80ToSystemCycles();
                        md_main.BeginZ80ResetCycle();
                        md_main.g_md_music?.YmStart();
