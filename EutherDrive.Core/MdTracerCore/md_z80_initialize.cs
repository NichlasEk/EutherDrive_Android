...
namespace EutherDrive.Core.MdTracerCore
{
    internal partial class md_z80
    {
        ...
        public void initialize()
        {
            ...
            ValidateOpcodeTable(g_operand_ddcb, nameof(g_operand_ddcb));
            ValidateOpcodeTable(g_operand_fd,   nameof(g_operand_fd));
        ...
        private static void ValidateOpcodeTable(Action[] table, string name)
        {
            ...
            {
                Debug.Fail($"{name} måste ha 256 element (har {(table == null ? 0 : table.Length)})");
                throw new InvalidOperationException($"{name} måste ha 256 element.");
            }
        }
    }
}
