...
namespace EutherDrive.Core.MdTracerCore
{
    ...
    {
        private void analyse_ROXL_reg()
        {
            // ROXL Dx, Dy or ROXL #<data>, Dy
            // Format: 1110 1sss 1mm0 rrrr
            ...
            // ROXL - Rotate Left through X (bits rotate through X flag)
            for (int i = 0; i < wcnt; i++)
            {
                g_clock += 2;
                bool msb = (g_work_data.l & MOSTBIT[w_size]) != 0;
                g_work_data.l = (g_work_data.l << 1);
                if (g_status_X) g_work_data.l |= 0x01; // X flag goes to LSB
                g_status_X = msb;
                g_status_C = msb;
            }
            ...
            // V flag: always cleared for ROXL
            g_status_V = false;
        }
    }
}