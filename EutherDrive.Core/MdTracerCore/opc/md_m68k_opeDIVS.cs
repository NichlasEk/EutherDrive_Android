    ...
    internal partial class md_m68k
    {
        private void analyse_DIVS()
        {
            ...
            if (g_work_data.w == 0)
            {
                ...
                g_status_N = false;
                g_status_Z = false;
                RaiseException("DIV0", 0x0014);
                return;
            }
            g_work_val1.l = (uint)((int)g_reg_data[g_op1].l / (short)g_work_data.w);
            if (((int)g_work_val1.l < -32768)|| (32767 < (int)g_work_val1.l)) { g_status_V = true; }
            else {
                g_status_V = false;
                g_work_val2.l = (uint)((int)g_reg_data[g_op1].l % (short)g_work_data.w);
