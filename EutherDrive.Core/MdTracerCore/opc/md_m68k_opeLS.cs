...
namespace EutherDrive.Core.MdTracerCore
{
    internal partial class md_m68k
    {
        private void analyse_LS_reg()
        {
            ...
            if (_madouRotateTrace && pcBefore == 0x013A58)
            {
                string dir = (w_dr == 0) ? "LSR" : "LSL";
                string size = w_size == 0 ? "B" : (w_size == 1 ? "W" : "L");
                uint after = g_reg_data[g_op4].l;
                Console.WriteLine($"[MADOU-LS] PC=0x{pcBefore:X6} {dir}.{size} cnt={wcnt} D{g_op4} before=0x{before:X8} after=0x{after:X8}");
            }
        }
        private void analyse_LS_mem()
        {
            int w_size = 1;
            int w_dr = g_op2 & 0x04;
            g_clock = 9;
