...
namespace mame
{
    ...
    public abstract class device_z80daisy_interface : device_interface
    {
        ...
        public void z80daisy_decode(uint8_t opcode)
        {
    ...
    public class z80_daisy_chain_interface : device_interface
    {
        ...
        void daisy_init(z80_daisy_config [] daisy)
        {
            ...
            while (tailptr != null && tailptr.m_daisy_next != null)
            ...
            for ( ; daisy[daisyIdx].devname != null; daisyIdx++)  //for ( ; daisy->devname != nullptr; daisy++)
            {
                ...
                device_z80daisy_interface previous = null;
                for (device_z80daisy_interface current = m_chain; current != null; current = current.m_daisy_next)
                {
                    if (current == intf)
                    {
                        if (previous == null)
                            m_chain = current.m_daisy_next;
                        else
                            previous.m_daisy_next = current.m_daisy_next;
                        break;
                    }
                ...
                intf.m_daisy_next = null;
                if (m_chain == null)
                    m_chain = intf;
                else
                    tailptr.m_daisy_next = intf;
                tailptr = intf;
            }
        ...
        public int daisy_update_irq_state()
        {
            for (device_z80daisy_interface intf = m_chain; intf != null; intf = intf.m_daisy_next)
            {
        ...
        public void daisy_call_reti_device()
        {
            for (device_z80daisy_interface intf = m_chain; intf != null; intf = intf.m_daisy_next)
            {
                int state = intf.z80daisy_irq_state();
