...
namespace mame
{
                                ...
                                //device_nvram_interface
    {
        ...
        protected virtual bool device_nvram_interface_nvram_read(util.read_stream file)
        {
            determine_final_base();

            size_t actual;
            std.error_condition err = file.read(m_base, m_length, out actual);
            return !err && actual == m_length;
        }

        protected virtual bool device_nvram_interface_nvram_write(util.write_stream file)
        {
            determine_final_base();

            size_t actual;
            std.error_condition err = file.write(m_base, m_length, out actual);
            return !err && actual == m_length;
        }
