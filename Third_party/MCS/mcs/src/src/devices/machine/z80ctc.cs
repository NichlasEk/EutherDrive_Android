...
namespace mame
{
                                 ...
                                 //device_z80daisy_interface
    {
        ...
        {
            return m_channel[(int)(offset & 3)].op0.read();
        }
        ...
        protected virtual int device_z80daisy_interface_z80daisy_irq_ack()
        {
            for (int ch = 0; ch < 4; ch++)
            {
                z80ctc_channel_device channel = m_channel[ch].op0;
                if ((channel.m_int_state & Z80_DAISY_INT) != 0)
                {
                    LOG("CTC IRQAck ch{0}\n", ch);
        ...
        protected virtual void device_z80daisy_interface_z80daisy_irq_reti()
        {
            for (int ch = 0; ch < 4; ch++)
            {
                z80ctc_channel_device channel = m_channel[ch].op0;
                if ((channel.m_int_state & Z80_DAISY_IEO) != 0)
                {
                    LOG("CTC IRQReti ch{0}\n", ch);
                    channel.m_int_state &= unchecked((u8)~Z80_DAISY_IEO);
                    interrupt_check();
                    return;
                }
            }
