...
namespace mame
{
    public static class XTAL_global
    {
        ...
        {
            if (clock.EndsWith("_Hz_XTAL")) return new XTAL(double.Parse(clock.Replace("_Hz_XTAL", ""), CultureInfo.InvariantCulture));
            else if (clock.EndsWith("_kHz_XTAL")) return new XTAL(double.Parse(clock.Replace("_kHz_XTAL", ""), CultureInfo.InvariantCulture) * 1e3);
            else if (clock.EndsWith("_MHz_XTAL")) return new XTAL(double.Parse(clock.Replace("_MHz_XTAL", ""), CultureInfo.InvariantCulture) * 1e6);
