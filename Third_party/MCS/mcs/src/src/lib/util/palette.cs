    ...
    public class palette_client : IDisposable
    {
        ...
        class dirty_state
        {
            ...
            public void resize(uint32_t colors)
            {
                ...
                // Mask unused bits in the last word. When the palette size is an
                // exact multiple of 32, the whole last word is valid.
                uint32_t remaining_bits = colors % 32;
                if (remaining_bits != 0)
                    m_dirty[dirty_dwords - 1] &= (1U << (int)remaining_bits) - 1;
            ...
            public void reset()
            {
                ...
                {
                    for (uint32_t entry32 = m_mindirty / 32; entry32 <= m_maxdirty / 32; entry32++)
                        m_dirty[entry32] = 0;
                }
        ...
        public void Dispose()
        {
            ...
            palette_client previous = null;
            for (palette_client client = m_palette.m_client_list; client != null; client = client.m_next)
            {
                if (client == this)
                {
                    if (previous == null)
                        m_palette.m_client_list = m_next;
                    else
